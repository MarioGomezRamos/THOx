...
C*IA  Q
	...
	GO TO 1
C
	ENTRY WIGN3J(A,B,C,XX,YY,ZZ)
	IERR=0
	KEY=1
	GOTO 1
C
	ENTRY RACAHC(A,B,C,XX,YY,ZZ)
	KEY=12
	IERR=0
	GOTO 100
C
	ENTRY WIGN6J(A,B,C,XX,YY,ZZ)
	KEY=11
	IERR=0
	GOTO 100
C
	ENTRY JAHNUF(A,B,C,XX,YY,ZZ)
	KEY=13
	IERR=0
	GOTO 100
C
...
C
  120 JS=K1-K2+K6
	IF(JS.LT.0) JS=-JS
	JSPAR=IPARF(JS)
	CLEBSG=JSPAR*CLEBSH/ SQRT(K3+one)
	RETURN
C
  320 IF(KEY.GE.10) GOTO 130
	...
	IZ=IZ+IY
	CLEBSH=IJPAR*Y*Z*ten**IZ
	GOTO(220,720),KEY
C
  720 JQ=K2-K1
	IF(JQ.LT.0) JQ=-JQ
	IJPAR=IPARF(JQ)
	CLEBSG=CLEBSH*IJPAR* SQRT(K3+one)
	RETURN
C
  420 CLEBSH=zero
	IERR=1
