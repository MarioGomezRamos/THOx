...
c changed to complex in v2.3
      ...
      complex*16 :: fprod
      complex*16 xi,pot_omp,xsum, xsumn,xsumc
...
!$OMP PARALLEL DO PRIVATE(iquad,irad2,nq,k,potn,potc,r1,r2,nc) 
!OMP& FIRSTPRIVATE(coefc,coefv,ncoul,nquad,nrad2,nqmin,nqmax,kmax,rquad,
!OMP& xrad2) SHARED(potQKc,potQKn)
      do iquad=1,nquad
      do irad2=1,nrad2
      do nq=nqmin,nqmax
      do k=0,kmax
...
! ncoul=2: Coulomb only  
      ...
      if ((ncoul.eq.0).or.(ncoul.eq.1)) then  !nuclear part
        ...
        potn= pot_omp(r1,r2,nq,k,coefc,nc)
     &      + pot_omp(r1,r2,nq,k,coefv,nc)
      ...
      if ((ncoul.eq.0).or.(ncoul.eq.2)) then ! Coulomb part
       nc=2 ! Coulomb part
       potc= pot_omp(r1,r2,nq,k,coefc,nc)
     &     + pot_omp(r1,r2,nq,k,coefv,nc)
      endif       
...
c nc=1: nuclear part
c nc=2: coulomb part
      complex*16 function pot_omp(r1,r2,iq,k,coef,nc)
      use xcdcc,only:nquad,xquad,wquad
      implicit real*8 (a-h,o-z)
      complex*16 ss,func_omp
      integer nc
!      write(*,*)'pot: nc=',nc
      ind=mod(nquad,2)
      ...
      do j=1,nquad/2
      dx=xquad(j)
      ss=ss+wquad(j)*(func_omp(dx,nc,iq,k,coef,r1,r2)+
     & func_omp(-dx,nc,iq,k,coef,r1,r2))
      enddo
      pot_omp=ss
      return
      end
...
c nc=1: nuclear part
c nc=2: coulomb part
      complex*16 function func_omp(xx,nc,iq,k,coef,r1,r2)
...
!        vcoreip=fival(xrvec,rfrag,vcorei(:,iq),nr,alpha)
      ...
      func_omp=Vq*fleg(xx,k)/(2.d0*xrvec**iq) !c.f. Eq. (22) of Summers 
      endif
