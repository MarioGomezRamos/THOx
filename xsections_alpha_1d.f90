...
!!! TESTING
      ...
      complex*16:: fxytab(6),fint2d,fint2dd,fint2db_jin
...
!     If no amplitudes have been calculated before, we try to read them
!     from an external file, specified by the user 
...
! *** Assign global index to states of all j/pi sets iPS=iPS(iset,n)
      iex=0
...
!*** Overlaps between PS's and Scattering states  -----------------------------
...
!*** Compute DOUBLE x-sections dsigma/(dEdW) FOR SELECTED CORE STATE ICORE
      if (ncont.gt.0) then
...
! Integrate in ENERGY, to get dsigma/dOmega for core state ICORE
...
! Integrate in ANGLE, to get dsigma/dE  for core state ICORE
...
!c---------------------------------------------------------------------------------
!     Start calculation of triple differential cross sections 
...
!*     ---------------------------------------------------------------
      ...
      if (ios /= 0) then 
         write(*,*)' Namelist gridthetac not present'
         rewind(kin)
         read(kin,nml=gridener)
      endif

      read(kin,nml=gridthetav,iostat=ios) !For alpha and beta this will be used for the cuts
      
      if (ios /= 0) then 
         write(*,*)' Namelist gridthetav not present'
         rewind(kin)
         read(kin,nml=gridener)
      endif
...
!*     ---------------------------------------------------------------           
      ...
      dphir=dphi*degrad
!*     ---------------------------------------------------------------
      write(77,307) nth,dth*degrad,thmin,thmin+(nth-1)*dth,dth
      write(77,307) itv,dar,al,au,da
      write(77,307) iten,dEn,Enlow,Enup
      write(77,307) itphi,dphir,phil,phiu,dphi
      
      if (phixs) then
      write(777,307) nth,dth*degrad,thmin,thmin+(nth-1)*dth,dth
      write(777,307) itv,dar,al,au,da
...
!      write(*,*)'energ(1,1)=',energ(1,1)
      ...
      if (jpiset(iset)%nho.eq.0) then ! bin set
        if (binset(iset)%kmax.gt.kmax) kmax=binset(iset)%kmax
      else                            ! PS set
...
!*     loop over core particle detection angle
     ...
     &           nth*itv*iten*lr8/1e6,'MB' !MGR  
      allocate(xs3body(iten,itv,nth),energ3(iten))
...
!AMM 
     ...
     &           nth*itv*iten*itphim*lr8/1e6,'MB' !AMM
      ...
      allocate(energ4(iten,itv,nth,itphim))
      xs3body_phi=0.0
      end if
...
!*     ---------------------------------------------------------------
!     loop over center of mass energy
      do 30 ien=1,iten
      En=Enlow+(ien-1)*dEn
...
!*     ---------------------------------------------------------------
!*     center of mass angle at 0 degrees
!*     ---------------------------------------------------------------
...
!     Compute kcl and kvl for the cuts
      ...
      Ev=hc**2*dot(kvl,kvl)/(2.d0*mv)

      if (tcu.gt.1e-6) then
      tcd=acos(kcl(3)/m(kcl))/degrad
      if (tcd .lt. tcl .or. tcd .gt. tcu) then
      tmatsq=0d0
      goto 500
      endif
      endif
      ...
      if (tvu.gt.1e-6) then
      tvd=acos(kvl(3)/m(kvl))/degrad
      if (tvd .lt. tvl .or. tvd .gt. tvu) then
      tmatsq=0d0
      goto 500
      endif
      endif
...
!     ---------------------------------------------------------------
      ...
      do inu=1,2*ilval+1
      phas=phas*phask
! AMM: Use explicit forms for PLM (l<9); otherwise, original recursive formula
       if (ilval.le.10) then
...
!       sumps=sumps+fv(n)*raux*famps0(iex,nial+ith,iii)
      sumps=sumps+fv(n)*famps0(iex,ith,iii)
...
!!!!
      ...
      enddo !  n (PS's)
      tmat(iset,inc,iii)=sumps !For PS get the t matrix directly without interpolating
...
!      do ith=0,5
        iex=ips(iset,n)
        fxyc(n)=famps0(iex,ith,iii)*sqrt(pi/2.)*wkfac(iset) ! includes phase e^(-i delta_el)
...
!     &  (famps0(iex,nial+ith,iii),ii=0,5)


      enddo !n 
      if2c=if2c+1
!      tmat(iset,inc,iii)=f2c(xb,yb,xyt,fxyc,6,nex,nord,10,maxne) ! CHECK DIMS HERE!!!!!!!!!!!!!!!!
       tmat(iset,inc,iii)=cfival(yb,xyt,fxyc,nex,0d0)
...
!      ampt=ampt+(-xi)**li*cgc(ind)*ylm(li,inu)*  ! NOTE phbin() inserted here!!!!!
      caux=(-xi)**li*cgc(ind)*ylm(li,inu)*tmat(iset,inc,iam)
      ampt=ampt+caux!*exp(xi*nint(mo-rnu-sig-mu)*phKb)
      ...
      enddo ! imu
      enddo ! imo
      tmatsq=(4.d0*pi/(mkp+1e-10))**2*tmatsq/dble(nmi)
      if (tmatsq.ne.tmatsq) then
      write(*,*)'NaN in tmatsq! Check CGC, ylm, or tmat'
      write(*,*)'tmatsq=',tmatsq
      call flush(6)
      tmatsq=0d0
      endif
...
!     -----------------------------------------------------------------
       xs3body(ien,iv,ith)=xsig

! AMM July 2019
       if (phixs) then
       do ip=1,itphim
       xs3body_phi(ien,iv,ith,ip)=sigphi(ip)
       enddo !ip
       endif
...
!$OMP END PARALLEL DO
      ...
      write(*,*)'Dims=',nth,itv,iten,itphim
      print 510
!MGR--------------------------------------------------------------------
      icount=0
      do ith=1,nth
        do iv=1,itv
          do ien=1,iten
            write(77,315) ien,iv,ith,xs3body(ien,iv,ith),energ3(ien)
            do ip=1,itphim
            icount=icount+1
            if (phixs) write(777,318) ien,iv,ith,ip,                    &
     &      xs3body_phi(ien,iv,ith,ip),energ3(ien),energ4(ien,iv,ith,ip)
            enddo !ip
          enddo
        enddo
      enddo    
