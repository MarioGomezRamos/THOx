      ...
      integer :: rank_telp = 0
      complex*16, allocatable :: sum_wU(:) ! numerator:  sum_J (2J+1) * V_off * psi_exc * psi_gs*
      real*8,     allocatable :: sum_w(:)  ! denominator: sum_J (2J+1) * |psi_gs|^2
      complex*16, allocatable :: utelp(:) ! constructed TELP potential U_TELP(R)

      contains

      subroutine accumulate_telp(nch, inc, wf, jtot)
        use nmrv, only: nr, vcoup
        integer, intent(in) :: nch, inc
        complex*16, intent(in) :: wf(nch, nr)
        real*8, intent(in) :: jtot
        integer :: ir, n
        complex*16 :: telp_num

        if (iftelp_val .and. rank_telp.eq.0) then
          do ir=1,nr
            telp_num = (0d0,0d0)
            do n=1,nch
              if (n.eq.inc) cycle
              telp_num = telp_num + vcoup(inc,n,ir) * wf(n,ir)
            enddo
            sum_wU(ir) = sum_wU(ir)
     &                 + (2d0*jtot+1d0)*telp_num*conjg(wf(inc,ir))
            sum_w(ir)  = sum_w(ir)
     &                 + (2d0*jtot+1d0)*abs(wf(inc,ir))**2
          enddo
        endif
      end subroutine accumulate_telp
...
c     ----------------------------------------------
      complex*16, intent(out):: wf(nch,npt)
...
c ... TEST (delete after debugging)
      real*8:: yaux(nch,nch)
c     .........................................................
      complex*16, intent(out):: wf(nch,nch,npt)
      complex*16 :: phase(nch),smat(nch)
...
c     --------------------------------------------------------------
      ...
      real*8 :: phr,cph(0:lmax) !!!! changed to lmax check !!!!
      real*8 :: fpmax,small,acc8,flux,magn(n1)
      real*8 :: whit_rescale,asympt_whit
c     ----------------------------------------------------------------
      complex*16:: wf(nch,n1)
      complex*16:: yd(nch,nch),yaux(5)
      complex*16:: y(nch,nch,n1)
...
c     --------------------------------------------------------------
      ...
      real*8:: y(nch,nch,n1),yop(nch,nop,n1),yor(nch,nop,n1)
      real*8:: ccoef,dcoef,gaux(nch,n1),amod,rcoef,eph(nch)
...
c ... 
     ...
     & npt,wf,phase,smat,method,info,einc,icc)
      use xcdcc, only: smats
      use nmrv,only: nr,h,vcoup,conv,ech,debug,rmin,hort,rvec,cutr
      use telp_mod, only: accumulate_telp
...
!      call matching2(ecm,z12,nch,ql,lmax,inc,rmatch,y,wf,phase,smat,info) ! LAPACK 
      ...
      if (incvec(ich)) then
      inc=ich
      call matching3(ecm,z12,nch,ql,lmax,inc,nr,y,wf,phase,smat,info)
      smats(icc,inc,1:nch)=smat(1:nch)
!c --- TELP accumulation ---
      call accumulate_telp(nch, inc, wf, jtot_loc)
      endif
      enddo
       call cpu_time(finish)
       tmatch=tmatch+finish-start
...
c ------------------------------------------------------
     ...
     & npt,wf,phase,smat,method,info,einc,icc)
      use xcdcc, only: smats
      use nmrv,only: nr,h,vcoup,conv,ech,debug,rmin,hort,rvec,rmort
      use telp_mod, only: accumulate_telp, iftelp_val
      use channels, only: jptset
      use globals, only: verb
      use factorials
      use constants , only: e2
